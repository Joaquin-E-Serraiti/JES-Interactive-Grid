...
function octileDistance(squareIndex1,squareIndex2,columns) {

    const x1 = squareIndex1%columns;
    const y1 = Math.floor(squareIndex1/columns);
    const x2 = squareIndex2%columns;
    const y2 = Math.floor(squareIndex2/columns);
...
export async function aStarDiagonal(grid) {
    skipList = new Map()
    maxLevel = 3;
    skipList.set("start",{
        id:"start",
    ...
    let currentNode = {index:start,parentId:null,gCost:0,hCost:octileDistance(start,end,grid.COLUMNS)};
    ...
    while (currentNode.index !== end) {
        ...
        const neighboursIndices = grid.getNeighbouringSquares(currentNode.index,[grid.STATES.EMPTY,grid.STATES.END],true);
        ...
        for (let i = 0; i<2; i++) {
            for (const neighbourIndex of [orthogonal,diagonal][i]) {
                ...
                const newNode = {index:neighbourIndex,parentId:currentNode.index,gCost:currentNode.gCost+([10,14][i]), hCost:octileDistance(neighbourIndex,end,grid.COLUMNS)};
...
function distanceToFarthestCorner(start,columns,rows) {
    ...
    return (manhattanDistance(start,cornerIndex,columns)/2)
}
