...
async function expansionCycle(originPointsMap,maxId,newOriginPoints,grid) {
    ...
    const log2MaxId = Math.log2(maxId[0]); // This is calculated only for visualization.
    while (currentPointId !== null) {
        ...
        if (newLayer !== null) {
            ...
            for (const index of newLayer) {
                if (!grid.algorithmCanRun) {return}
                //grid.colorSquare(index,"rgb(82, 104, 214)")

                grid.colorSquare(index,`rgb(${60+(19*log2MaxId)},${80+(17*log2MaxId)},${230-(1.8*log2MaxId)})`)
            }
    ...
    while (newOriginPoints.length === 0) {
        ...
        if (grid.visToggleOn){
            if (grid.visualizationSpeed === 1) {
                ...
                await grid.delay(0);
            } else if (grid.visualizationSpeed === 4 && (expansionsCounter%6)===0) {
                await grid.delay(0);
            }
        }
        // -------------------------------------------------------------
